...
// ─── Section Tree Node (multi-select) ────────────────────────────────────────
    ...
    <div>
      <div
        ...
        style={{
          ...
          padding: "5px 10px", paddingLeft: `${12 + depth * 18}px`,
          cursor: "pointer", borderRadius: "5px", userSelect: "none",
          backgroundColor: isSelected ? "color-mix(in srgb, var(--accent) 12%, transparent)" : "transparent",
      ...
      >
        {children.length > 0 ? (
          <span
            ...
            style={{ fontSize: "0.65rem", opacity: 0.5, width: "12px", flexShrink: 0 }}
          >
            {expanded ? "v" : ">"}
          </span>
        ) : (
        ...
        <div style={{
          ...
          fontSize: "0.6rem", color: "#fff", transition: "all 0.15s",
        }}>
          {isSelected ? "✓" : ""}
        </div>
      ...
      {expanded && children.map(c => (
        <SectionNode key={c.id} section={c} depth={depth + 1}
          selectedIds={selectedIds} onToggle={onToggle} childrenMap={childrenMap} />
      ))}
    </div>
  );
}
    ...
    <label style={{ display: "flex", alignItems: "flex-start", gap: "10px", cursor: "pointer" }}>
      <div
        ...
        style={{
          ...
          border: "1px solid", cursor: "pointer", position: "relative", transition: "background 0.2s",
          backgroundColor: checked ? "var(--accent)" : "var(--bg-inset)",
...
function FormatPill({ value, selected, onSelect, label }) {
  return (
    <button onClick={() => onSelect(value)} style={{
      ...
      backgroundColor: selected ? "var(--accent)" : "var(--bg-panel)",
...
// ─── Shared Section Picker (multi-select) ────────────────────────────────────
  ...
  return (
    <div style={s.card}>
      ...
      <p style={{ margin: 0, fontSize: "0.78rem", color: "var(--text-muted)" }}>
        {count === 0 ? "Exporting all sections." : `${count} section${count !== 1 ? "s" : ""} selected.`}
      </p>
      {loading && <div style={s.dimText}>Loading sections...</div>}
      {error   && <div style={s.errorText}>{error}</div>}
      {!loading && sections.length > 0 && (
        <div style={s.sectionTree}>
          {rootSections.map(sec => (
            <SectionNode key={sec.id} section={sec} depth={0}
              selectedIds={selectedIds} onToggle={onToggle} childrenMap={childrenMap} />
          ))}
        </div>
      )}
      {!loading && sections.length === 0 && !error && (
        <div style={s.dimText}>No sections found.</div>
      )}
    </div>
  );
}
...
const ALL_COLUMNS = [
  ...
  { key: "custom_user_type_classic_or_next", label: "Account Type" },
  { key: "custom_tc_user_type",              label: "User Type" },
  ...
  { key: "custom_tc_automation_status",      label: "Automation Status" },
];

function ExportTab({ credentials, selectedProject, selectedSuite,
  sections, rootSections, childrenMap, loadingSections, sectionError }) {
  ...
  async function handleExport() {
    setLoading(true); setError(""); setSuccess(""); setProgress("");
    ...
    try {
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        const secName = sections.find(s => s.id === secId)?.name || "all sections";
        setProgress(`Fetching ${secId ? secName : "all sections"} (${i + 1}/${targets.length})...`);
        const res = await axios.post(
          `${BASE_URL}/api/tools/export-csv`,
          { ...credentials, project_id: selectedProject.id, suite_id: selectedSuite?.id || null,
            section_id: secId, strip_html: stripHtml, include_links: includeLinks },
          { responseType: "blob" }
        );
      ...
      if (!headerLine) { setError("No data returned."); setLoading(false); setProgress(""); return; }

      const label = selectedIds.size === 0 ? "all"
        : selectedIds.size === 1 ? sections.find(s => s.id === [...selectedIds][0])?.name?.replace(/\s+/g, "_") || "section"
        : `${selectedIds.size}_sections`;
      const name = `testrail_export_${label}.csv`;
  ...
  const sectionLabel = selectedIds.size === 0 ? "All"
    : selectedIds.size === 1 ? sections.find(s => s.id === [...selectedIds][0])?.name || "1 section"
    : `${selectedIds.size} sections`;

  return (
    <div style={s.twoCol}>
      <div style={s.col}>
        <SectionPicker
          sections={sections} rootSections={rootSections} childrenMap={childrenMap}
          selectedIds={selectedIds} onToggle={toggleSection}
          onSelectAll={selectAll} onClearAll={clearAll}
          loading={loadingSections} error={sectionError}
        />
      </div>
      <div style={s.col}>
        <div style={s.card}>
          <div style={s.cardTitle}>Format</div>
          <div style={{ display: "flex", gap: "8px" }}>
            <FormatPill value="CSV"  selected={format === "CSV"}  onSelect={setFormat} label="CSV" />
            <FormatPill value="XLSX" selected={format === "XLSX"} onSelect={setFormat} label="Excel (XLSX)" />
          </div>
        </div>
        <div style={s.card}>
          <div style={s.cardTitle}>Options</div>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            <Toggle checked={stripHtml}    onChange={setStripHtml}    label="Strip HTML tags"       hint="Cleans markup from rich-text fields" />
            <Toggle checked={includeLinks} onChange={setIncludeLinks} label="Include TestRail links" hint="Adds a direct link column per case" />
          </div>
        </div>
        <div style={s.card}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: "4px" }}>
              <button style={s.microBtn} onClick={() => setSelectedCols(ALL_COLUMNS.map(c => c.key))}>All</button>
              <button style={s.microBtn} onClick={() => setSelectedCols([])}>None</button>
            </div>
          </div>
            ...
            {ALL_COLUMNS.map(col => {
              ...
              return (
                <label key={col.key} style={{
                  padding: "4px 10px", borderRadius: "20px", fontSize: "0.74rem",
                  cursor: "pointer", userSelect: "none", transition: "all 0.15s",
                  border: `1px solid ${on ? "var(--accent)" : "var(--border)"}`,
                  backgroundColor: on ? "color-mix(in srgb, var(--accent) 15%, transparent)" : "transparent",
                  color: on ? "var(--accent)" : "var(--text-muted)", fontWeight: on ? 600 : 400,
                }}>
                  <input type="checkbox" checked={on} onChange={() => toggleCol(col.key)} style={{ display: "none" }} />
                  {col.label}
                </label>
              );
            })}
        ...
        <div style={s.card}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "8px" }}>
            {[{ k: "Sections", v: sectionLabel }, { k: "Columns", v: selectedCols.length }, { k: "Format", v: format }].map(({ k, v }) => (
              <div key={k} style={{ textAlign: "center", padding: "8px", borderRadius: "6px", backgroundColor: "var(--bg)", border: "1px solid var(--border)" }}>
                <div style={{ fontSize: "0.88rem", fontWeight: 700, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{v}</div>
                <div style={{ fontSize: "0.7rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "2px" }}>{k}</div>
              </div>
            ))}
          </div>
          ...
          <button style={{ ...s.actionBtn, opacity: canExport ? 1 : 0.4 }} onClick={handleExport} disabled={loading || !canExport}>
            {loading ? <><span style={s.spinner} /> {progress || "Exporting..."}</> : `Download ${format}`}
          </button>
        </div>
      </div>
    </div>
...
function FindBlankTab({ credentials, selectedProject, selectedSuite,
  sections, rootSections, childrenMap, loadingSections, sectionError, onOpenCase }) {
  ...
  function toggleField(key) {
    setCheckedFields(prev => prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]);
  }
    ...
    try {
      for (let i = 0; i < targets.length; i++) {
        ...
        const res = await axios.post(`${BASE_URL}/api/cases/`, {
          ...
          suite_id: selectedSuite?.id || null, section_id: secId, offset: 0, limit: 250,
        });
      ...
      setCasesError(err?.response?.data?.detail || "Failed to load cases.");
    }
  ...
  function handleExportCSV() {
    const headers = ["Test Case ID","Test Name","Title","Use Case","Test Steps","Expected Result","Issues","TestRail ID","Link"];
    const rows = blankCases.map(c => {
      const issues = checkedFields
        .map(f => { const iss = getIssueLabel(c, f); return iss ? `${BLANK_FIELDS.find(b => b.key === f)?.label}: ${iss}` : null; })
        .filter(Boolean).join("; ");
      const cid = c.id || "";
      return [
        c.custom_tc_test_case_id || "", c.custom_tc_name || "", c.title || "",
        stripHtmlTags(c.custom_tc_use_case), stripHtmlTags(c.custom_steps), stripHtmlTags(c.custom_expected),
        issues, cid, cid ? `${credentials?.url || ""}/index.php?/cases/view/${cid}` : "",
      ];
    });
    const csv = [headers, ...rows].map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const a = Object.assign(document.createElement("a"), { href: URL.createObjectURL(blob) });
    a.download = `blank_cases_${selectedIds.size === 1 ? sections.find(s => s.id === [...selectedIds][0])?.name?.replace(/\s+/g, "_") || "section" : "all"}.csv`;
    document.body.appendChild(a); a.click(); a.remove();
  }
        ...
        <div style={s.card}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {BLANK_FIELDS.map(({ key, label }) => {
              ...
              return (
                <div key={key} onClick={() => toggleField(key)} style={{
                  display: "flex", alignItems: "center", gap: "10px", cursor: "pointer",
                  padding: "8px 10px", borderRadius: "7px", transition: "background 0.12s",
          ...
          <p style={{ margin: 0, fontSize: "0.75rem", color: "var(--text-muted)" }}>Cases shown if any checked field matches criteria.</p>
        </div>
        <div style={s.card}>
          <div style={s.cardTitle}>Word Count Filter</div>
          <Toggle checked={wordCountEnabled} onChange={setWordCountEnabled} label="Enable word count threshold" hint="Also flag fields that are too short" />
          {wordCountEnabled && (
            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
              <span style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>Flag fields with fewer than</span>
              <input type="number" min={1} value={wordCountThreshold}
                onChange={e => setWordCountThreshold(Math.max(1, parseInt(e.target.value) || 1))}
                style={{ width: "58px", padding: "4px 8px", borderRadius: "6px", border: "1px solid var(--border)", backgroundColor: "var(--bg-panel)", color: "var(--text)", fontSize: "0.88rem", textAlign: "center" }} />
      ...
      <div style={s.col}>
        {!searched && !loadingCases && (
          <div style={s.emptyState}>
            <div style={{ fontSize: "0.9rem", color: "var(--text-muted)", textAlign: "center", lineHeight: 1.6 }}>
              Configure filters on the left, then click Find Cases.
            </div>
          </div>
        )}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "8px" }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}>
                  ...
                  <span style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>
                    {blankCases.length === 1 ? "case" : "cases"} found{cases.length > 0 && ` out of ${cases.length} total`}
                  </span>
                </div>
                {blankCases.length > 0 && (
                  <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    <FormatPill value="CSV"  selected={exportFormat === "CSV"}  onSelect={setExportFormat} label="CSV" />
                    <FormatPill value="XLSX" selected={exportFormat === "XLSX"} onSelect={setExportFormat} label="XLSX" />
                    <button style={{ ...s.microBtn, padding: "6px 14px", fontSize: "0.8rem", whiteSpace: "nowrap" }} onClick={handleExportCSV}>Export</button>
                  </div>
                )}
              </div>
                    ...
                    <div key={c.id} style={s.caseRow}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px" }}>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          ...
                          {c.custom_tc_name && <div style={{ fontSize: "0.76rem", color: "var(--text-muted)", marginBottom: "5px" }}>{c.custom_tc_name}</div>}
                            ...
                            {issueFields.map(f => {
                              ...
                              return (
                                <span key={f} style={{
                                  ...
                                  backgroundColor: isBlank ? "rgba(239,68,68,0.12)" : "rgba(249,115,22,0.12)",
                                  color: isBlank ? "#ef4444" : "#f97316",
                                  border: `1px solid ${isBlank ? "rgba(239,68,68,0.3)" : "rgba(249,115,22,0.3)"}`,
                                }}>{lbl}: {issue}</span>
                              );
                            })}
                          </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "4px", flexShrink: 0 }}>
                          {onOpenCase && (
                            <button onClick={() => onOpenCase(c)} style={{ padding: "4px 10px", borderRadius: "6px", fontSize: "0.76rem", border: "1px solid var(--border)", color: "var(--text-muted)", backgroundColor: "var(--bg)", cursor: "pointer", whiteSpace: "nowrap" }}>
                              Open
                            </button>
                          )}
                          <a href={`${credentials?.url || ""}/index.php?/cases/view/${c.id}`} target="_blank" rel="noreferrer"
                            style={{ padding: "4px 10px", borderRadius: "6px", fontSize: "0.76rem", border: "1px solid var(--border)", color: "var(--text-muted)", textDecoration: "none", whiteSpace: "nowrap", backgroundColor: "var(--bg)", textAlign: "center" }}>
                            Open Externally
                          </a>
                        </div>
                      </div>
...
export default function ExportCases({ credentials, selectedProject, selectedSuite, onOpenCase }) {
  ...
  async function loadSections() {
    ...
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/projects/${selectedProject.id}/sections`,
        { ...credentials }, { params }
      );
      ...
      raw.forEach(sec => {
        if (!sec.parent_id) roots.push(sec);
        else { (byParent[sec.parent_id] ??= []).push(sec); }
      });
      setChildrenMap(byParent); setRootSections(roots);
    } catch (err) {
      setSectionError(err?.response?.data?.detail || "Failed to load sections.");
    }
    ...
    <div style={s.root}>
      <div>
        ...
        <p style={{ margin: "2px 0 0", fontSize: "0.82rem", color: "var(--text-muted)" }}>
          {selectedProject ? `${selectedProject.name}${selectedSuite ? ` / ${selectedSuite.name}` : ""}` : "No project selected"}
        </p>
      </div>
...
const s = {
  ...
  caseRow:   { backgroundColor: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: "8px", padding: "12px 14px" },
};
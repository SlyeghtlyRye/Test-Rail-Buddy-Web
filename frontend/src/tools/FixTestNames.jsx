...
function toName(title) {
  return (title || "Untitled").replace(/\s+/g, "_");
}
...
export default function FixTestNames({ credentials, selectedProject, selectedSuite, selectedSection, sections = [] }) {
  ...
  // Reset preview when section or resetAll changes
  useEffect(() => {
    setPreviewCases([]);
    setResults(null);
    setError("");
  }, [overrideSection, selectedSection, resetAll]);
  ...
  // ── Determine which cases need fixing ───────────────────────────────────
  function needsFix(c) {
  ...
  const handleFix = async () => {
    ...
    try {
      ...
      if (caseIds.length === 0) { setError("No cases need fixing."); setLoading(false); return; }
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.description}>
        Replaces spaces with underscores and fills blank test names from the case title.
      </p>
      ...
      {previewCases.length > 0 && (
        <div style={styles.previewBox}>
          <p style={styles.previewHeader}>
            Preview —{" "}
            <span style={{ color: "#22c55e" }}>{toFix.length} to fix</span>
            {toSkip.length > 0 && (
              <span style={{ color: "var(--text-dim)", marginLeft: "8px" }}>{toSkip.length} skipped</span>
            )}
          </p>
          <div style={styles.previewList}>
            {previewCases.map(c => {
              const oldName = c.custom_tc_name || "";
              const newName = toName(c.title);
          ...
          <div style={styles.previewActions}>
            ...
            <button
              style={{ ...styles.btn, opacity: toFix.length === 0 ? 0.4 : 1 }}
              onClick={handleFix}
              disabled={loading || toFix.length === 0}
            >
        ...
        <div style={styles.results}>
          <p style={styles.resultSummary}>
            ...
            {results.results?.filter(r => r.skipped).length > 0 && (
              <span style={{ color: "var(--text-dim)" }}> &nbsp;|&nbsp; — Skipped: {results.results.filter(r => r.skipped).length}</span>
            )}
            {results.errors > 0 && (
...
const styles = {
  ...
  previewRow: {
    display: "grid", gridTemplateColumns: "160px 20px 200px 1fr",
    alignItems: "center", gap: "8px", padding: "3px 4px", borderRadius: "4px",
    minWidth: 0,
  },
